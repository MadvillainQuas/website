...
| 1c — the league's channel, no key | **built, verified** | RSS feed of the channel (`youtube_channel` in the registry) + the watch page's `startTimestamp`/`endTimestamp`; a fixture carries its scheduled stream days ahead, the anchor completes when the stream starts and the log has a tip. 7 of 8 BCB games of 5–6 Sep matched (the eighth was never streamed). |
| 1d — the clock track (vision model / scoreboard reader) | **built, needs 0099** | `trackClock` reads the overlay every 5 s through a local copy; `positionFromTrack` places every play by period + clock (first reading of a clock = the whistle); `set_video_clock_track` RPC stores `game_videos.clock_track`; import accepts `epinoia-clock-track/1` from any model. Tested on synthetic tracks; not yet on a full broadcast. |
