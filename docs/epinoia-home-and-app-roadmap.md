...
- **Global fixtures, one dropdown per league (Louie, 2026-09-25).** The page shows only the leagues with a game in the next 7 days (one light read, `weekLeagues`: ids and tip-offs, counted per league; a league with a live game is there too), each as its own dropdown with its week's count. A league's games are read when its dropdown is opened (the top one is opened for the reader) by the same two cursors scoped to that league (`feed({ league })`, a filter on the inner-embedded season's league), 8 nearest to now at a time, and **Show more is the league's own**, at the foot of its list, with "8 of 47" beside it. There is no page-wide Show more; live games are still pinned above every dropdown.
