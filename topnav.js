...
(function () {
    ...
    function build() {
        ...
        // Decide which is "the other app"
        const path = (window.location.pathname || '').toLowerCase();
        const isShortlist = path.endsWith('basketball-analyzer-profiles_9.html');
        const isLineup    = path.endsWith('lineup.html') || path.endsWith('index_9.html');
        const otherHref = isShortlist ? 'lineup.html' : 'basketball-analyzer-profiles_9.html';
        const otherLabel = isShortlist ? '🏀 Lineup Analyzer' : '⭐ Prophesy Shortlist';
        ...
        other.className = 'nav-link';
        other.href = otherHref + (adminParam && isShortlist === false ? '' : ''); // future hook
