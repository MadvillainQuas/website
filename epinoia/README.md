...
Lives under `/epinoia/` in `MadvillainQuas/website`. Built on the Backlit kit.
...
* `localhost:8741/epinoia/devfeed.html` — press **Start live game**
* `localhost:8741/epinoia/game/?g=demo&mode=local` — watch it update
...
* **The Supabase SDK is vendored** (`vendor/supabase.js`) rather than loaded from a CDN, keeping
  the promise of no third-party scripts under `/epinoia/`. CI excludes `vendor/` from credential
