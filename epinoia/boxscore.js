...
function playerAdvTable(d,t,TA,gameAvg,ranges){
  ...
  /* THE GAME PAGE DRAWS EACH PLAYER AS A CARD (cards.js) from the same rows, ranges and percentiles */
  const CK = globalThis.EpinoiaCards;
  if (CK && CK.players) return CK.players({d, t, TT, OT, rows, ranges, gameAvg, groups:ADV_GROUPS, S, tname, gpRate, sort:advSort});
...
function bxTeamHTML(d,t){
  ...
  /* THE GAME PAGE DRAWS EACH ROW AS A CARD (cards.js); the scorer keeps the table */
  const CK = globalThis.EpinoiaCards;
  if (CK && CK.box) return CK.box({d, t, T, S, five, rest, ordered, cols, teamChips:teamChipsHTML(d,t), tname, fmtMin});
  const line = p=>{
    const s = d.stats[p.id];
...
function advHTML(d){
  ...
  /* THE GAME PAGE MAY DRESS EACH CHART AS A CARD (epinoia/game/cards.js, EpinoiaCards): the row goes in
     as drawn and comes back inside a card with the difference between the two sides under it. The scorer
     does not load it, so its own screens draw exactly as they always have. `o.plain` opts a row out (the
     tempo tiles are tiles already). */
  const mirror = (label, h, a, max, fmt, hWin, aWin, k, hTxt, aTxt, o) => {
    const row = '<div class="mrrow">'+gpv(k?rate(k,0):null, hTxt!=null?hTxt:fmt(h), 'ffval'+(hWin?' winner':''), label)+
    ...
    const CK = globalThis.EpinoiaCards;
    return (CK && CK.chart && !(o && o.plain)) ? CK.chart(row, {label, h, a, fmt, hWin, aWin, k, TA, S, c0, c1, tname}) : row;
  };
  ...
  const ffRow = (x, plain)=>{ const h=TA[0][x.k], a=TA[1][x.k]; const hw = h>a, aw = a>h;
    const hWin = x.hb==null ? false : (x.hb?hw:aw), aWin = x.hb==null ? false : (x.hb?aw:hw);   // hb null = neither is "better"
    return mirror(x.l, h, a, x.max, x.f, hWin, aWin, x.k, undefined, undefined, plain ? {plain:true} : undefined); };
  ...
  const ffRows = FF.filter(x=>TEMPO.indexOf(x.k)<0).map(x=>ffRow(x)).join('');
  const tempo = FF.filter(x=>TEMPO.indexOf(x.k)>=0).map(x=>'<div class="ftile" data-k="'+x.k+'">'+ffRow(x,true)+'</div>').join('');
