...
function clubPanel(host, o) {
  ...
  /* ---- the crest ----
     THE CLUB'S OWN MARK, and the one thing on a club page that is unmistakably
     theirs. Until now every club was drawn as a monogram over its colour —
     which is a real design and not a placeholder, and stays as the fallback,
     because a club with no crest must still look like something. This puts a
     crest over it where a club has one.

     It goes through the media queue like every other image rather than into a
     column of its own: a crest is published on a public page under a league's
     name, and the league approving what appears under its name is the whole
  ...
  crestFile.addEventListener('change', async () => {
    ...
    crestPick.disabled = true;
    try {
      const up = await window.EpinoiaUpload.upload(o.sb, {
        file: f, ownerType: 'team', ownerId: o.team.id, kind: 'logo' });
      if (!up || !up.storage_path) throw new Error('the upload returned no path');
      crestPrev.textContent = '';
      const img = el('img'); img.alt = '';
      img.src = window.EpinoiaUpload.publicUrl(window.EPINOIA_CONFIG, up.storage_path);
      crestPrev.appendChild(img);
      crestPrev.appendChild(el('p', 'note',
        'Uploaded. Your crest replaces the monogram once the league approves it.'));
      o.say('Crest uploaded and queued for approval.', 'ok');
    } catch (e) {
      o.say('Upload failed: ' + (e.message || e), 'err');
    }
    crestPick.disabled = false;
  });
