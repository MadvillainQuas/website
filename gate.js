 ...
 * localStorage for INVITE_VALID_DAYS), the page is hidden and
 * a generic "site not configured" message is shown instead.
 *
...
(function () {
    ...
    // To rotate: choose a new code, sha256 it, and replace the hex below.
    //   echo -n "<new code>" | openssl dgst -sha256
    const INVITE_HASH_HEX  = 'b56aad31ab2d5f0426f4ca1baf42f53b0626f9159bb83f2665c0087f9892ac70';
    const INVITE_VALID_DAYS = 60;
    const STORAGE_KEY      = 'prophesy_invited_v1';
    ...
    function setUnlocked() {
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ ts: Date.now() })); } catch (_) {}
    }
    ...
    function isUnlocked() {
        try {
            ...
            return ageMs >= 0 && ageMs <= INVITE_VALID_DAYS * 24 * 3600 * 1000;
