 ...
 *
 * Global suspension: the admin can flip config/gate-suspended.json
 * to suspended:true to OPEN the site for any visitor without an
 * invite code. While suspended, gate.js silently writes the current
 * invite hash into each visitor's localStorage. When suspension is
 * lifted later, those browsers still hold the matching hash and
 * stay unlocked — i.e. everyone who showed up during the open
 * window is auto-granted permanent access. Toggle via the admin
 * dashboard's "Suspend invite gate" card.
 *
...
(function () {
    ...
    // Fetch the admin-managed suspension flag. Returns true if the
    // site is currently in "open-access" mode (every visitor auto-
    // granted). Returns false on missing/malformed file so the gate
    // fails closed — i.e. an unreachable suspension config does NOT
    // accidentally open the site. Cache-busted because we want the
    // newest value on every page load.
    async function isGateSuspended() {
        try {
            const res = await fetch('config/gate-suspended.json?t=' + Date.now(), { cache: 'no-cache' });
            if (!res.ok) return false;
            const j = await res.json();
            return !!(j && j.suspended === true);
        } catch (_) { return false; }
    }

    async function check() {
        // Already unlocked? Fast path — no network needed.
        if (isUnlocked()) {
            reveal();
            return;
        }

        // GLOBAL SUSPENSION — admin has opened the door for everyone.
        // We persist the unlock so when the gate flips back on later
        // this visitor stays in without needing the invite link.
        if (await isGateSuspended()) {
            setUnlocked();
            reveal();
            return;
        }
